    ...
    $('.member-grade-txt-btn').on({
        click(e){
            ...
            $.ajax({
                url:'./data/member.txt',
                type:'GET',
                dataType:'TEXT', //XML, CSV, TEXT, JSON
                success(res){                    
                    console.log( res );
                    // 1. 줄단위로 분리하고
                    //    줄끝을 검색  => \r\n      
                    ...
                    for(let i=0; i<arr.length; i++){  // 줄(Row)
                        $('.tbody-grade-text').append(`<tr  data-key="${arr[i][0]}">`);
                        for(let j=0; j<arr[i].length; j++){ // 칸(Column)
                            $('.tbody-grade-text').append(`<td>${arr[i][j]}</td>`);
                        }
                        $('.tbody-grade-text').append(`</tr>`);
                    }
                    // $('.tbody-grade-text').listview("refresh"); // 바인딩 새로고침 jQuery 모바일 전용
