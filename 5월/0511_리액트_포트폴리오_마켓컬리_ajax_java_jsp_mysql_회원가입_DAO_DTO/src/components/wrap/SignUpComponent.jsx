...
// 9. 생년월일
// 생년 월 일
// 숫자 말고 한글 또는 특수문자 전부 안됨 숫자만 가능
// 0. 숫자만 입력 나머지 모두 제거 => 정규표현식

// 함수 제작 그리고 생년, 생월, 생일 연결 => 유즈이팩트 활용
// useEffect()는 상태관리 변수 변경되면 즉각 실행
// 1. 모든 칸이 빈칸이면 오류 없음 초기화 return
// 2. 년(100세이하) 채워지면 => 월 입력 요구 - 태어난 월을 정확하게 입력해주세요
// 3. 년(14세미만) 채워지면 => 월 입력 요구 - 만 14세 미만은 가입이 불가합니다.
// 4. 년(미래년도 안된다.) 채워지면 => 월 입력 요구 - 태어난 월을 정확하게 입력해주세요
// 5. 월(1-12) 채워지면 => 일 입력 요구 - 태어난 일을 정확하게 입력해주세요 
...
const birthCheckFn=()=>{
    ...
    if( state.생년==='' && state.생월==='' && state.생일==='' ){
        isBirthMsg = '모두 빈칸이어서 오류 메시지 삭제하고(초기화) 리턴 나간다.';
     isBirth = false;
     isBirthMsg = '';
    }
    else{
        //  생년 제한 조건 디버깅 큰숫자 -> 중간숫자 -> 작은 숫자
        // 1). 년(미래년도 안된다.) 채워지면 => 월 입력 요구 - 태어난 월을 정확하게 입력해주세요 큰숫자 범위
        // 2). 년(14세미만) 채워지면 => 월 입력 요구 - 만 14세 미만은 가입이 불가합니다. 중간 숫자 범위
        // 3). 년(100세이하) 채워지면 => 월 입력 요구 - 태어난 월을 정확하게 입력해주세요 낮은숫자 범위
        // 2023-100 1923 > 1922 안된다
        isBirth = true;
        // 2023-100 1923 1922
        if( state.생년.length < 4 ){ // 생년 (4자리 숫자)
            isBirth = true;
            isBirthMsg = ' 태어난 년도를 4자리를 정확하게 입력해주세요 ' ;
        }

        else if( Number(state.생년) > nowYear ){ // 년(미래년도 안된다.)
            isBirth = true;
            isBirthMsg = '생년월일 확인하세요(미래년도)';
            
        }

        else if( Number(state.생년) >= (nowYear-14 )){ // 년(14세미만) 나이가 어린 나이
            isBirth = true;
            isBirthMsg = '만 14세 미만은 가입이 불가합니다.';
        }

        else if( Number(state.생년) < (nowYear-100) ){ // state.생년(100세이하)
            isBirth = true;
            isBirthMsg = '생년월일 확인하세요(100세 초과)';
        }
...
const onChangeUserBirthDate=(e)=>{
    ...
    // 11. 이용약관동의
    // 11-1 전체 동의 체크 박스onChange() 이벤트
    // 개별체크해서 이용약관동의 항목이 7개이면 체크한다.
    // 7개 미만이면 체크 해제
    const onChangeUserServiceAll=(e)=>{
        let 이용약관동의 = [];

        if(e.target.checked===true){  // 전체 동의를 체크 했다면
           
            이용약관동의 = state.이용약관 // 이용약관 배열 7개의 항목 모두 => // 이용약관 동의 배열에 저장한다.
        }
        else{
            이용약관동의 = []; // 빈 배열 삭제
        }
    ...
    const onChangeUserService=(e)=>{
        ...
        if(e.target.checked===true){ // 체크되면 개별체크 선택항목 value 값을 이용약관동의 배열에 저장시킨다.
           ...
           // 현재 SNS를 체크했는데 이메일이 체크 되어있고 그리고 무료배송, 할인쿠폰 등 혜택/정보 수신 동의(선택) 선택이 안되어 있다면
            ...
            //  1. 무료배송, 할인쿠폰 등 혜택/정보 수신 동의(선택) 해지하면 : state.이용약관동의 배열 값 3개 모두 삭제
                // 삭제1 [state.이용약관동의 ] ;무료배송, 할인쿠폰 등 혜택/정보 수신 동의(선택) => state.이용약관동의 제거 후 결과 => 임시배열에 저장한다.
                // 삭제2 [이용약관동의] : SNS 
                // 삭제3 [이용약관동의]:이메일 모두 해제 한다.
            if(e.target.value === '무료배송, 할인쿠폰 등 혜택/정보 수신 동의(선택)'){
                ...
                setState({
                    ...state,
                    이용약관동의:이용약관동의 // 삭제 되고 새롭게 배열 구성 완료
                })
            }
            ...
            // 현재 체크해제한게 이메일이고 SNS가 체크되어 있다면 이메일만 삭제
...
SignUpComponent.defaultProps = {
    회원가입: {
       ...
       // 이용약관동의 : 내용이 체크박스 VALUE 값과 같다면 체크 된다.
       이용약관동의:[],
