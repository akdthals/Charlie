(function($, window, document){ // 내부변수
    ...
    const wedding = {
        ...
        header(){
            ...
            //$(scrolltop스크롤 이벤트는 스크롤이 이벤트가 발생 되어야만 동작한다.
            //$(window).resize(); 창크기

            let newScroll = null; $(window).scrollTop(); //새로운 스크롤 값
            let oldScroll = null; newScroll;
        ...
        section1(){
            ...
            function pageNation(){
                ...
                $('.page-btn').removeClass('on');      // 페이지 버튼 모두 클래스 삭제
                $('.page-btn').eq(cnt>2 ? 0 : cnt).addClass('on'); // 현재 슬라이드 번호 클래스 추가
            }
