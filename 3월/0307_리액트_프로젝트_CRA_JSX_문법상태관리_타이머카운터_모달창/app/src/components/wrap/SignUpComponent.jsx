...
export default function SignUpComponent({timerClear,isTimerFn, setId,seconds,minutes,timerMsg}){
    ...
    const onClickHpOkNumber=(e)=>{
        ...
        setState({
            ...state,
            isModal : true,
            modalMsg: `인증번호가 전송되었습니다. ${hpOkNumber}`,
            hpOkNumber : hpOkNumber, 
            isTimer: true // 모달창 닫기할 때 true이면 타이머 동작 시작한다.
        })
                ...
                <form   name='sign_up' id='signUp' method='post' action='./sign_up.php' >
                    <ul>
                        ...
                        <li>
                            <div>
                                ...
                                <button 
                                    ...
                                    type='button' 
                                    disabled={state.isUserHp} /* 버튼 isUserHp 변수값이 false 이면 사용가능 */
                        ...
                        <li>
                            <div>
                                <input 
                                onChange={onChangeUserHpOk} type="text" name='user_hpOk' id='userHpOk' placeholder='휴대폰 인증번호 입력하세요'/>
                                <button 
                                    onClick={onClickOkIf} 
                                    type='button'  
                                    ...
                                    >인증번호 확인</button>
                                    <span>{minutes}:{seconds}</span>   {/* 2<10?`0${2}`: */}
                            </div>
