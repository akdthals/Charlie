    ...
    const onClickHpNum=(e)=>{
        ...
        if(regExp.test(state.휴대폰)===false){
           ...
           confirmMsg = '잘못된 휴대폰 번호 입니다. 다시 한번 확인해 주세요';
            ...
            // 인증번호 전송 타이밍
            num = Math.floor ( Math.random()*9000000+100000); // 인증번호: 6자리의 난수 발생
            isConfirmModal = true;
            confirmMsg = `인증번호가 발송 되었습니다. ${num} `;
        }
        setState({
            ...state,
                                ...
                                <ul>
                                    ...
                                    <li>
                                        <div>
                                            ...
                                            <button className={`hp-num-btn ${state.isHpdisabled?'':' on'}`}onClick={onClickHpNum2} disabled={state.isHpdisabled2} type='button'  >인증번호 받기</button>
                                            <p className={`error-msg ${state.isHpError?' on':''}`}>{state.isHpMsg}</p>
                                        </div>
                                    </li>
                                        )
...
SignUpComponent.defaultProps = {
    회원가입: {
        ...
        isHpdisabled: true, // 휴대폰 인증번호 받기 버튼 사용 불가 (true)
        발송인증번호:'',
