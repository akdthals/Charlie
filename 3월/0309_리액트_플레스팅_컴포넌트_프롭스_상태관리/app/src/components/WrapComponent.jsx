...
class WrapComponent extends React.Component {
    ...
    // 생성자 컨스트럭터
    constructor(props){
        super(props);  // 슈퍼를 사용하여 => this 사용가능
        this.state = {
            title: '클래스형 컴포넌트 타이틀',
            cnt: 0,
            과일: this.props.좋아하는과일,
            회원: {
                아이디:'',
                비밀번호:'',
                이름:'',
    ...
    // 스토레이지 저장
    // 1. 추가된 배열에 목록을 브라우저 저장에서 저장한다.( 영구 저장된다. )
    ...
    onClickTodoListAddStorage=(e)=>{
        ...
        // 문제점 : 새로고치고 배열이 비어 있는상태 또는 입력된 1개의 데이터 인경우
        // 스토레이지에 저장하면 스토레이지 데이터가 업데이트 되면선 모두 삭제되고 빈데이테 또는
        // 최근 입력된 1개 이상의 데이터만 저장된다.(덮어쓰기)
        // 스토레이지 이전에 있는 데이터를 그대로 보관하고 배열에 있는 데이터는 추가로 저장한다.
        // 그럴려면  스토레이지 데이터를 가져오기 하고 추가된 배열 데이터를 병합한다. 전개연산상를 이용
        // 1. 로컬스토레이지 데이터 가져오기(게터)
        const getData = localStorage.getItem( key );
        // 예외처리 빈값 null 널은 오류가 아니다. 
        // 그러나 잠정적으로 이후에 오류가 발생할 수 있는 이유가된다
    ...
    render() {
        ...
        console.log( `this.props ${this.props.좋아하는과일}` );
        // // 1씩 증가 클릭 이벤트 함수 :  상태 변경 세터함수 이용
        // const onClickAdd=(e)=>{
        //     e.preventDefault();
        //     this.setState({
        ...
        return (
            <div id='wrap'  style={{textAlign:'center', lineHeight: '200%'}}>
                ...
                {/* 컨스트럭터 함수 클릭 이벤트 : 반드시 this사용 이벤트 */}
                ...
                <br /><br /><br />
                {/* 클릭 이벤트와 매개변수 전달 구현 */}
                <button onClick={(e)=>this.onClickParamAdd(e, '1씩증가')}>클릭이벤트 매개변수 전달 구현 1씩증가버튼</button>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                <button onClick={(e)=>this.onClickParamSub(e, '1씩감소')}>클릭이벤트 매개변수 전달 구현  1씩감소버튼</button>
                ...
                <div style={{width:'300px',margin: '50px auto'}}>
                    {
                        this.state.isGender===true ? <img src={female} alt="예쁜 여성 이미지" style={{width:'100%'}} /> : <img src={male} alt="멋진 남성 이미지"  style={{width:'100%'}} />
                    }
                     ...
                     <div className="container">
                        ...
                        <form name='todo' id='todo' method='post' action="./todo.php">
                            ...
                            <div className="btn-box">
                                ...
                                &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                                <button  onClick={this.onClickTodoListAddStorage}>로컬스크레이지 저장</button> 
                            </div>
                        </form>
                    </div>                   
                </div>    
