...
export default function SignUpComponent({회원가입, timer, timerCounterfn}) {
    ...
    // 유즈 Ref
    // const createRef2 = React.createRef(); // 입력상자 태그요소 휴대폰을 선택함.(참조함)
    ...
    // useRef는 새로고침해야한 증가 값 보인다.
    // useRef는 새로고침해도 값이 유지된다.

    // state는 새로고침하면 새로 시작 초기화
    // state는 변경되면 화면이 다시 그려진다. 마운트 된다.
    ...
    //    ([영문+(1자이상)]+[특수문자+(1자이상)]+)+     2가지이상 조합 그룹2 이거나,
    //    ([숫자+(1자이상)]+[특수문자+(1자이상)]+)+     2가지이상 조합 그룹3 이거나
    // 3) 공백 사용 안됨
    // 4) 한글 사용 안됨
    ...
    const onChangeUserPw1=(e)=>{
        ...
        //const regExp1 = /.{10,16}/g;   /* true이면 정상 */ // 헐렁한 수량자
        const regExp1 = /^(.){10,16}$/g;   /* true이면 정상 */ // 엄격한 수량자
        // const regExp2 = /()()()/g; 3그룹
        // const regExp2 = /()+|()+|()+/g; 3그룹 셋중에 한그룹 반드시
        // const regExp2 = /([]+[]+)+|([]+[]+)+|([]+[]+)+/g; 2가지 이상 조합
        // /([영문숫자])+|([숫자영문])+|([영문특수문자])+|([특수문자영문])+|([숫자특수문자])+|([특수문자숫자])+/g;
        // const regExp2 = /([영문]+[숫자]+)+|([숫자]+[영문]+)+|([영문]+[특수문자]+)+|([특수문자]+[영문]+)+|([숫자]+[특수문자]+)|([특수문자]+[숫자]+)+/g;
        const regExp2 = /([A-Za-z]+[0-9]+)+|([0-9]+[A-Za-z]+)+|([A-Za-z]+[`~!@#$%^&*()\-_=+[{\]}\\|;:'",<.>/?]+)+|([`~!@#$%^&*()\-_=+[{\]}\\|;:'",<.>/?]+[A-Za-z]+)+|([0-9]+[`~!@#$%^&*()\-_=+[{\]}\\|;:'",<.>/?]+)+|([`~!@#$%^&*()\-_=+[{\]}\\|;:'",<.>/?]+[0-9]+)+/g;  //true이면 정상
        const regExp3 = /[가-힣ㄱ-ㅎㅏ-ㅣ]/g;  // 한글 false 이면 정상  한글이 들어가면 안되서
        const regExp4 = /\s/g; // 공백 사용 불가능 (\s < 공백)  공백 false이면 정상 공백이 들어가면 안되니깐
        const regExp5 = /(\d)\1\1/g; // false이면 정상 // 동일한 숫자 연속 3자이상  연속 사용불가
        ...
        if(regExp1.test(value)===false){
            ...
            isPwError = true;
            isPwMsg = '영문/숫자/특수문자(공백제외)만 허용하며, 2개이상 조합 ';
        }
        else if(regExp5.test(value)===true){
            isPwError = true;
            isPwMsg = '동일한 숫자 연속 3자이상 연속 사용불가 ';
        }
        else{
            isPwError = false;
            isPwMsg = '';
    ...
    // 비밀번호와 비밀번호 확인 입력상자 입력값 다르면 : 동일한 비밀번호 입력
    const onChangeUserPw2=(e)=>{
        const {value} = e.target;
         let isPw2Error = false;
         let isPw2Msg = '';

        // 현재 입력값과 이전에 입련된 비밀번호를 비교한다.
        if(value===''){
             isPw2Error = true;
    ...
    const onChangeName=(e)=>{
       ...
       setState({
        ...
        // 공백 안됨
        // cshong123@naver.com
        // csho-ng123@naver.com
        // cs_hon.g123@naver.com
        // cshon.g123@na_ver.com
        // cshon.g123@na_ve.r.com
        // cshon..g123@na_ve.r.kr  .1자 사용 점 dot  점 두개는 안됨.
        // ? 1자 또는 0자
        // (\.)? 닷(dot) 1자 아니면 또는 0자 
        // ^ 시작 
        // [^0-9] 숫자가 아닌것 곽괄호 안에는 부정
        // $ 끝 

        // 맨앞(영문숫자한글)필수(.)?(영문숫자한글)선택@ (영문숫자한글-_.)필수 \.영문만 {2, 3} 맨뒤
        // 제외 @  (  )  \  [  ]  "  ;   :   ,   <   >  
        // 사용 가능한 특수문자 사용 ` ~ ! @ # $ % ^ & * - _ + { } | ' / \. ?
        // /^ []+  (\.)?  []*  @  []+  \.[]  {2,3}  $/gi; ignore 이그노 영문대소문자 구별 없음
    ...
    const onClickHpNum=(e)=>{
        e.preventDefault();
        // 휴대폰 번호가 정확한지 정규표현식으로 유효성 검사 정규 표현식
        // ^010-2764-6281$
        // 010-456-6281
        // 011-456-6281
        // 012-456-6281
        ...
        // 018-456-6281
        // 019-456-6281
        // 010 011 012 015 016 017 018 ~ 019 맨앞세자리
        ...
        if(regExp.test(state.휴대폰)===false){
            ...
            isConfirmModal = true;
           confirmMsg = '잘못된 휴대폰 번호 입니다. 다시 한번 확인해 주세요';
        }
        else{
            ...
            isConfirmModal = true;
            confirmMsg = `발송인증번호가 발송 되었습니다. ${num} `;
        }
        setState({
            ...state,
    ...
    // 6-2 휴대폰발송인증번호확인 입력상자 onChange() 이벤트
    const onChangeHp2=(e)=>{
        const {value} = e.target;
    ...
    const onClickHpNum3=(e)=>{
        ...
        // 1. 맨위 루트영역에 ref를 생성하고 변수 createRef 에 저장한다.
        // 2. 생성된 createRef 변수를 태그요소인 휴대폰 입력상자에 ref=(createRef) 에 대입시킨다.
        // 3. 이벤트 수행시 휴대폰 입력상자에 커서를 포커스 시킨다. createRef.current.focus();
        createRef.current.focus(); //커서를 휴대폰 입력상자에 맞추겠다. 커서가 깜빡깜빡 거린다.
    }
    ...
    const onClickConfirmModalClose=(e)=>{
        ...
        // console.log(state.confirmMsg.includes('발송인증번호'))  // 찾으면 true, 못 찾으면 flase
        // console.log(state.confirmMsg.indexOF('발송인증번호'))  // -1 찾지 못하면 - 찾으면 >= 0
        // console.log(state.confirmMsg.serch('발송인증번호'))  // -1 찾지 못하면 - 찾으면 >= 0
        e.preventDefault();
                            ...
                            <form name='sign_up_form' id='signUpForm' method='post' action="./sign_up.php">
                                <ul>
                                    ...
                                    <li>
                                        ...
                                        <div className="center-box">
                                            <input onChange={onChangeUserPw1} 
                                            type="password" 
                                            maxLength={16} 
                                            name='user_pw' 
                                            id='userPw' 
                                            ...
                                            state.발송인증번호!== '' && (
                                                    <li>
                                                        <div>
                                                            ...
                                                            <button className={`hp-num-btn ${state.isHpdisabled2?'':' on'}`}onClick={onClickHpNum2} disabled={state.isHpdisabled2} type='button'>인증번호 확인 버튼</button>
                                                        </div>
                                                    </li>
                                            )
                                        }
...
SignUpComponent.defaultProps = {
    회원가입: {
        ...
        isHpdisabled: true, // 휴대폰 발송인증번호 받기 버튼 사용 불가 (true)
        발송인증번호 : '',

        // 6-3 휴대폰 입력인증번호 확인 입력상자
        입력인증번호:'',
        isHpdisabled2: true, // 휴대폰 인증번호 확인버튼 사용 불가 (true)

        // 6-4 휴대폰 입력인증번호 확인 버튼 
       ...
       confirmMsg:'사용가능한 아이디입니다. 아이디는 16자이하로 사용가능합니다',
       isConfirmModal: false, // true 모달 열기 false 모달 닫기
    }
}